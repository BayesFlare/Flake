/* some tests of SSE versions of functions (C++)                                
...
int main(){
  ...
  std::vector<float> phases(N), phases2(N), phases3(N), sinphis(N);
  std::vector<float> ssephases(N), ssephases2(N), ssephases3(N), ssesinphis(N);
  ...
  clock_t begin = clock();
  for ( j = 0; j < Ntries; j++ ){ VectorMathSin( &ssesinphis[0], &ssephases[0], N ); }
  clock_t end = clock();
  ...
  begin = clock();
  for ( j = 0; j < Ntries; j++ ){ VectorMathExp( &ssephases3[0], &ssesinphis[0], N ); }
  end = clock();
  time_spent = (double)(end - begin) / CLOCKS_PER_SEC;
  ...
  begin = clock();
  for ( j = 0; j < Ntries; j++ ){ VectorMathAdd( &phases3[0], &phases[0], &phases2[0], N ); }
  end = clock();
  time_spent = (double)(end - begin) / CLOCKS_PER_SEC;
